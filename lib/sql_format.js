...
const ColumnFieldTypes = {
    fmt: Function,
    default: [Number, Boolean, String, Object, Array, Date],
    nullable: Boolean,
    is_unique: Boolean,
    is_primary: Boolean,
    is_indexed: Boolean,
...
const optFilter = (form, enable_undefined = false) => {
    // return: {is, is_not, eq}
    const opts = {}
    for (let key in form) {
        let value = form[key];
        ...
        if (value === null) {
            ...
            if (value === undefined) {
                if (enable_undefined) {
                    ...
                    console.warn(`[WARNING]: Query(${key}=undefined) is formatted as (${key} IS NOT Null) `)
                } else {
                    throw new SqlArgsError(`Invalid Filter(${key}=undefined)`)
                }
            } else {
...
const sqlFormat = (opts = {}, order = {}, limit) => {
    ...
    for (let mark in opts) {
        const op = K_OP[mark] || K_OP[mark.toLowerCase()];
        if (op === undefined) {
