...
// global singleton default config to SQL
const SqlConfig = {
    enable_undefined: false,
    allow_multiple_on_find_one: false,
}
...
class DbTable {
    ...
    async countAsync(cond = {}, ensureNotDeleted, enable_undefined) {
        let fks = Object.keys(cond);
        let key = fks[0] || "*";
        const form = this.constructor.queryForm(cond, ensureNotDeleted);
        if (enable_undefined === undefined) {
            enable_undefined = SqlConfig.enable_undefined;
        }
        const opts = optFilter(form, enable_undefined);
        const res = `count(${key}) as count`;
        const qry = {opts, res}
        const rows = await this.queryAsync(qry);
        return rows[0].count;
    }

    async findAsync(cond = {}, ensureNotDeleted, res = '*', order = {}, limit, enable_undefined) {
        const form = this.constructor.queryForm(cond, ensureNotDeleted);
        if (enable_undefined === undefined) {
            enable_undefined = SqlConfig.enable_undefined;
        }
    ...
    async findOneAsync(cond = {}, ensureNotDeleted, res = '*', order = {}, allow_multiple_on_find_one) {
        const rows = await this.findAsync(cond, ensureNotDeleted, res, order, 2)
        if (rows.length >= 2) {
            let msg = `${this.tablename}.findOneAsync(${JSON.stringify(cond)}) found results >= 2`;
            dbcPool.logger.warn(`[WARNING]: ${msg}`);
            dbcPool.logger.info(`[INFO]: DbTable.findOneAsync() is deprecated, suggest to use DbTable.getOrNoneAsync()`);
            if (allow_multiple_on_find_one === undefined) {
                allow_multiple_on_find_one = SqlConfig.allow_multiple_on_find_one
            }
            if (allow_multiple_on_find_one) {
                dbcPool.logger.warn(`[WARNING]: SqlConfig.allow_multiple_on_find_one = false`);
                throw new sql_errors.SqlMultipleResultsFound(`${msg} :\n ${ JSON.stringify(rows, null, 2)} `);
            }
        }
        return rows[0] || null
    }

    async getOrNoneAsync(cond = {}, ensureNotDeleted, res = '*', order = {}) {
        const rows = await this.findAsync(cond, ensureNotDeleted, res, order, 2)
        if (rows.length <= 1) {
            return rows[0] || null
        }
        let msg = `${this.tablename}.getOneOrNoneAsync(${JSON.stringify(cond)}) found results >= 2`;
        dbcPool.logger.error(`[ERROR]: ${msg} :\n ${ JSON.stringify(rows, null, 2)} `);
        throw new sql_errors.SqlMultipleResultsFound(msg)
    }

    async getOr404Async(cond = {}, ensureNotDeleted, res = '*') {
        const obj = await this.getOrNoneAsync(cond, ensureNotDeleted, res);
        if (obj === null) {
            throw new sql_errors.SqlDataNotFound(`table<${this.tablename}> data not found with condition(${JSON.stringify(cond)}>`);
        }
        return obj;
    }
