...
const {SqlError, SqlArgsError} = require('./sql_errors');
    ...
    getDbc: (name) => {
        ...
        if (!dbc) {
            ...
            if (cfg) {
                ...
                throw new SqlArgsError(`[dbcPool] no found dbc<${name}>`)
            }
        }
        return dbc;
    },
...
class DbTable {
    constructor(tablename, dbc) {
        ...
        if (dbc && dbc instanceof Object && dbc._cls === DbcName) {
            ...
            throw new SqlArgsError(`[DbTable:${this._cls}:${tablename}], init DbTable with invalid dbc ...`)
        }
    }
    ...
    async ensureColumnsAsync(...column_names) {
        ...
        for (let column_name of column_names) {
            ...
            if (!is_existed) {
                throw new SqlArgsError(`DbTable<${this._cls}> unknown field "${column_name}"`);
            }
        }
    }
    ...
    async findOneAsync(cond = {}, ensureNotDeleted, res = '*', order = {}) {
        const rows = await this.findAsync(cond, ensureNotDeleted, res, order, 1)
        return rows[0] || null;
    }
    ...
    async replaceManyAsync(objects, strict = true) {
        ...
        */
        if (objects instanceof Array) {
            ...
            if (count === 0) {
                throw new SqlArgsError(`DbTable<${this._cls}>: no objects to update`)
            }
            ...
            throw new SqlArgsError(`DbTable<${this._cls}>:  objects to replace must be Array`);
        }
    }
    ...
    async upsertManyAsync(objects, duplicate_update = {key: "deleted", value: false}, strict = true) {
        ...
        if (!key) {
            throw new SqlArgsError(`DbTable<${this._cls}>: duplicate_update(${JSON.stringify(duplicate_update)})`)
        }
        ...
        if (objects instanceof Array) {
            const count = objects.length;
            if (count === 0) {
                throw new SqlArgsError(`DbTable<${this._cls}>: no objects to upsert`)
            }
            ...
            throw new SqlArgsError(`DbTable<${this._cls}>: objects is not instanceof Array`);
        }
    }
