...
class DbTable {
    ...
    async findOneAsync(cond = {}, ensureNotDeleted, res = '*', order = {}, allow_multiple) {
        ...
        if (rows.length >= 2) {
            ...
            if (allow_multiple === undefined) {
                allow_multiple = SqlConfig.allow_multiple_on_find_one
            }
            if (!allow_multiple) {
    ...
    async getOrNoneAsync(cond = {}, ensureNotDeleted, res = '*') {
        const rows = await this.findAsync(cond, ensureNotDeleted, res, {}, 2)
        if (rows.length <= 1) {
            return rows[0] || null
        }
